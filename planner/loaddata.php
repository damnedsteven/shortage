...
$result = $mysqli->query('SELECT *, date_format(orderdate, "%d/%m/%Y") as orderdate, date_format(m.lastupdated, "%b %d %Y %h:%i %p") as lastupdated FROM master m LEFT JOIN pn p ON m.pn=p.pn WHERE status="1"');
$mysqli->close();
